...
export default function ProductPage({ 
  ...
  const allImages = galleryImages.length > 0 ? galleryImages : [productImage];
    ...
    <div className="container mx-auto px-4 py-8 animate-fade-in">
      ...
      <div className="flex justify-center mb-4">
        <Link to={packLink} className="hover:opacity-90 transition-opacity flex items-center gap-2 text-primary font-semibold">
          <ArrowLeft className="w-4 h-4" /> Back to {packName}
        </Link>
      </div>

      <h1 className="text-5xl font-bold font-headline text-primary mb-8 text-center">{productName}</h1>
            ...
            <CardContent className="p-6 space-y-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-secondary/30 p-3 rounded border border-secondary">
                    ...
                    <span className="text-sm">{supportedFormats.audio}</span>
                  </div>
                  <div className="bg-secondary/30 p-3 rounded border border-secondary">
                    <strong className="block text-primary mb-1">Video</strong>
                    <span className="text-sm">{supportedFormats.video}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-primary/5 p-4 rounded-lg border border-primary/10">
                ...
                <div className="space-y-2 text-sm">
                  {versions.map((v, idx) => (
                    <p key={idx}><span className="font-bold text-foreground">{v.name}:</span> {v.desc}</p>
                  ))}
                </div>
              </div>
            </CardContent>
